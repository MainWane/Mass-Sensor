...
void setupPins() {
  ...
  digitalWrite(RELAY_PIN, HIGH); // starter aktivt
  digitalWrite(GREEN_LED_PIN, HIGH);
  digitalWrite(YELLOW_LED_PIN, LOW);
  digitalWrite(RED_LED_PIN, LOW);
}

void handleRelayAndIndicators(float current) {
  ...
  digitalWrite(YELLOW_LED_PIN, (!emergencyShutdown && current > OVERCURRENT_THRESHOLD) ? HIGH : LOW);
...
void handleSensorReadings(unsigned long currentMillis) {
  if (currentMillis - lastSampleTime >= sampleInterval) {
    ...
    float current = ina219.getCurrent_mA() / 1000.0; // konverter til A
...
void loop() {
  ...
  handleRelayAndIndicators(current);
